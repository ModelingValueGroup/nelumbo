...
├── nelumbo-server          → Lean HTTP executor (JDK com.sun.net.httpserver + mvg-json, no third-party deps): NelumboServer serves a KB over REST (/eval, /eval/trace, /metadata, /health) + shared EvalService + KnowledgeBaseLoader/NamedSource + CLI Main; jar nelumbo-server-<version>.jar (~2 MB)
