...
- Releases: `.github/workflows/release.yaml` fires on `v*` tag pushes, downloads the build run's artifacts and attaches all artifacts in the uniform `nelumbo-<what>-<version>.{jar,zip}` pattern: `nelumbo-editor-*`, `nelumbo-cli-*`, `nelumbo-cli-server-*` (lean server), `nelumbo-web-server-*`, `nelumbo-mcp-server-*`, `nelumbo-eclipse-plugin-*`, `nelumbo-intellij-plugin-*`, `nelumbo-slides-*`; release body comes from `RELEASE_NOTES.md` (`${version}`/`${version-num}` placeholders).
