...
├── nelumbo-server          → Lean HTTP executor (JDK com.sun.net.httpserver + mvg-json, no third-party deps): NelumboServer serves a KB over REST (/eval, /eval/trace, /metadata, /health) + shared EvalService + KnowledgeBaseLoader/NamedSource + CLI Main; jar nelumbo-cli-server-<version>.jar (~2 MB)
...
- Releases: `.github/workflows/release.yaml` fires on `v*` tag pushes, downloads the build run's artifacts and attaches all artifacts in the uniform `nelumbo-<what>-<version>.{jar,zip}` pattern: `nelumbo-ide-*`, `nelumbo-cli-*`, `nelumbo-cli-server-*` (lean server), `nelumbo-web-server-*`, `nelumbo-mcp-server-*`, `nelumbo-eclipse-plugin-*`, `nelumbo-intellij-plugin-*`, `nelumbo-slides-*`; release body comes from `RELEASE_NOTES.md` (`${version}`/`${version-num}` placeholders).
