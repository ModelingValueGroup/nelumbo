...
The frontend is an npm project at `website/src/main/frontend/` (Monaco + monaco-languageclient, esbuild IIFE). It exposes `NelumboFields.connect()` (establish the single page-shared `/lsp` client) + `NelumboFields.mountFields(container)` (mount `.nelumbo-field` editors within a container, idempotent) so the feature tour mounts each section's editors lazily over one shared client; `initNelumboFields()` (the playground) mounts every field at once. An exercise field with a following `.nelumbo-solution` sibling gets a "Show solution" toggle. The LSP-connection-failed banner (`.nelumbo-lsp-banner`, prepended to `body`) is a fixed strip at the bottom of the viewport - it must stay position:fixed because the tour's `body` is a flex row and an in-flow banner pushes `main` offscreen. The `:website:npmBundle` Gradle task runs `npm run dist`; the bundle is registered as a source-set output dir and shipped inside the shaded jar under `public/assets/`, served at `/assets/`. See `website/src/main/frontend/README.md` for the (intentionally old, self-contained) dependency-stack rationale.

All three pages link `<link rel="icon" href="/favicon.svg">`: the real lotus logo recolored to the palette (`public/favicon.svg`, served by an explicit route in `NelumboHttpServer`). The pages are `landing.html` (served at `/`): a static intro page (no LSP/eval dependency; only external dependency is Google Fonts) linking to tour and playground. All three pages share the font stack (Google Fonts on each page): Bricolage Grotesque for headings, Instrument Sans for body, JetBrains Mono for code - also set as Monaco `fontFamily` in `nelumbo-fields.ts` (with a `monaco.editor.remeasureFonts()` on `document.fonts.ready` since the webfont can land after the first editor measures glyphs). The landing hero animation blooms five petals (the exact center-petal subpath of the logo; the others are copies rotated/scaled so each lands on an icon petal position) and then crossfades into the exact icon paths copied from `lsp/plugins/*/icons/logo.svg`. The petals' stroke weight (22), fill (`rgba(237,212,232,.1)`) and end transforms are tuned to match the icon render so the crossfade is seamless - keep them in sync if either side changes; the end frame must stay identical to the icon. The site-wide accent palette is purple, derived from the lotus logo (`lsp/plugins/*/icons/logo.svg`, petal `#EDD4E8`): accent `#c184d8`, soft `#edd4e8`, violet `#a99ae8`; landing uses dark aubergine backgrounds, tour/playground/`fields.css` use `#c184d8` as accent (no browns/golds/blues) - its messaging positions Nelumbo as a logic meta-language for specifying declarative languages/DSLs (syntax + semantics, semantics in logic, immediate line-n/line-n+1 availability, MPS/EMF replacement; logic is the medium, not the target), and its `grandparent.nl` code showcase is real verified Nelumbo (checked with the MCP `eval_nl` tool - keep it valid when editing); the landing e2e spec asserts the h1 contains "language"; `tour.html` (served at `/tour.html`): a sidebar feature tour with 8 sections, each with prose, a demo field, and self-checking exercises (the exercise query carries an expected result like `? [(r=120)][..]`, so the LSP mismatch squiggle confirms a solution) - below 760px the sidebar becomes an off-canvas drawer with a fixed hamburger toggle (`#nav-toggle`/`#nav-backdrop`; closes on section click or backdrop tap; desktop layout and the e2e selectors are untouched); and `playground.html` (served at `/playground.html`): a single free-form field.
