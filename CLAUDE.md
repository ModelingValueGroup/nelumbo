...
- Versioning: local (non-CI) builds are versioned `dev` (override in root `build.gradle.kts`); in CI mvgplugin rebases the `gradle.properties` version (a base/floor - fixed at 0.9.2, never below the lowest existing `v*` tag, see the comment there) to the next vacant tag version and pushes the `v*` tag. The Eclipse bundle's OSGi `Bundle-Version` needs a numeric version, so it falls back to the `gradle.properties` value locally. `playwright.config.ts` picks the newest `nelumbo-http-server-*.jar` by mtime.
