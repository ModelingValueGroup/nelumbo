...
├── nelumbo-server          → Lean HTTP executor (JDK com.sun.net.httpserver + Jackson, no third-party server): NelumboServer serves a KB over REST (/eval, /eval/trace, /metadata, /health) + shared EvalService + KnowledgeBaseLoader/NamedSource + CLI Main; jar nelumbo-server-<version>.jar (~4 MB)
