...
public final class Concat extends Predicate {
    ...
    public Concat(Functor fuctor, Object[] args) {
    ...
    protected InferResult infer(int nrOfUnbound, InferContext context) {
        ...
        String addend1 = getVal(1, 1);
        String addend2 = getVal(2, 1);
        String sum = getVal(3, 1);
        if (addend1 != null && addend2 != null) {
            ...
            if (sum != null) {
                ...
                return set(3, org.modelingvalue.nelumbo.strings.String.of(s)).factCI();
            }
        } else if (addend1 != null && sum != null) {
            if (sum.startsWith(addend1)) {
                return set(2, org.modelingvalue.nelumbo.strings.String.of(sum.substring(addend1.length()))).factCI();
            } else {
                return falsehoodCC();
            }
        } else if (addend2 != null && sum != null) {
            if (sum.endsWith(addend2)) {
                return set(1, org.modelingvalue.nelumbo.strings.String.of(sum.substring(0, addend2.length()))).factCI();
            } else {
                return falsehoodCC();
            }
        } else {
