 ...
 * (look and feel, token colors) and coordinates multiple editor windows through
 * the WindowManager.
 */
public class NelumboEditor {
    /**
     * Defines a color scheme for a token type with foreground and background
     * colors, and text style attributes (bold, italic, underline, subscript,
     * superscript).
     */
    public record ColorScheme(Color foreground, Color background, boolean bold, boolean italic, boolean underline,
            boolean subscript, boolean superscript, SimpleAttributeSet attr) {

        public ColorScheme(Integer fore, Integer back, boolean bold, boolean italic, boolean underline,
                boolean subscript, boolean superscript) {
            this(fore == null ? null : new Color(fore), back == null ? null : new Color(back), bold, italic, underline,
                    subscript, superscript, makeAttSet(fore, back, bold, italic, underline, subscript, superscript));
        }

        public ColorScheme(Color fg, Color bg, boolean bold, boolean italic, boolean underline, boolean subscript,
                boolean superscript) {
            this(fg, bg, bold, italic, underline, subscript, superscript, makeAttSet(fg == null ? null : fg.getRGB(),
                    bg == null ? null : bg.getRGB(), bold, italic, underline, subscript, superscript));
        }

        static SimpleAttributeSet makeAttSet(Integer fore, Integer back, boolean bold, boolean italic,
                boolean underline, boolean subscript, boolean superscript) {
    ...
    private static final String[] FONT_NAMES = { "consolas", "input mono", "dejavu sans mono", "overpass mono",
            Font.MONOSPACED };
    ...
    /**
     * Map from TokenType to ColorScheme defining how each token type should be
     * colored. This is mutable so users can customize colors.
     */
    ...
    /**
     * Example .nl files bundled with the application: {category, filename,
     * displayName}.
     */
    private static final String[][] EXAMPLE_RESOURCES = {
            ...
            { "Assignments", "transformationAssignment.nl", "Transformation Assignment" },
            // Answers
            ...
            { "Answers", "transformation.nl", "Transformation Answer" }, };
    ...
    private void initLookAndFeel() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Unable to set FlatLaf look and feel", "Warning",
    ...
    /**
     * Resolves an import name to its display name and resource path. Returns a
     * String array [displayName, resourcePath], or null if not found.
     */
    ...
    private void loadTokenColors() {
        for (TokenType tokenType : DEFAULT_TOKEN_COLORS.keySet()) {
            ...
            if (fgValue != null || bgValue != null || preferences.get(boldKey, null) != null
                    || preferences.get(italicKey, null) != null || preferences.get(underlineKey, null) != null
                    || preferences.get(subscriptKey, null) != null || preferences.get(superscriptKey, null) != null) {
    ...
    public void showColorConfigDialog(JFrame parent) {
        ...
        for (Map.Entry<TokenType, ColorScheme> entry : TOKEN_COLORS.entrySet().stream()
                .sorted(Comparator.comparingInt(e -> e.getKey().ordinal())).toList()) {
            TokenType tokenType = entry.getKey();
            ColorScheme scheme = entry.getValue();
            ...
            fgButton.addActionListener(e -> {
                ...
                Color newColor = JColorChooser.showDialog(dialog, "Choose Foreground Color for " + tokenType.name(),
                        initialColor);
                if (newColor != null) {
                    fgButton.setBackground(newColor);
                    fgButton.setForeground(getContrastColor(newColor));
                    TOKEN_COLORS.computeIfPresent(tokenType,
                            (k, currentScheme) -> new ColorScheme(newColor, currentScheme.background(),
                                    currentScheme.bold(), currentScheme.italic(), currentScheme.underline(),
                                    currentScheme.subscript(), currentScheme.superscript()));
                }
            });
            ...
            bgButton.addActionListener(e -> {
                ...
                Color newColor = JColorChooser.showDialog(dialog, "Choose Background Color for " + tokenType.name(),
                        initialColor);
                if (newColor != null) {
                    bgButton.setBackground(newColor);
                    bgButton.setForeground(getContrastColor(newColor));
                    TOKEN_COLORS.computeIfPresent(tokenType,
                            (k, currentScheme) -> new ColorScheme(currentScheme.foreground(), newColor,
                                    currentScheme.bold(), currentScheme.italic(), currentScheme.underline(),
                                    currentScheme.subscript(), currentScheme.superscript()));
                }
            });
            ...
            clearBgButton.addActionListener(e -> {
                ...
                TOKEN_COLORS.computeIfPresent(tokenType,
                        (k, currentScheme) -> new ColorScheme(currentScheme.foreground(), null, currentScheme.bold(),
                                currentScheme.italic(), currentScheme.underline(), currentScheme.subscript(),
                                currentScheme.superscript()));
            });
            ...
            boldCheckbox.addActionListener(e -> TOKEN_COLORS.computeIfPresent(tokenType,
                    (k, currentScheme) -> new ColorScheme(currentScheme.foreground(), currentScheme.background(),
                            boldCheckbox.isSelected(), currentScheme.italic(), currentScheme.underline(),
                            currentScheme.subscript(), currentScheme.superscript())));
            ...
            italicCheckbox.addActionListener(e -> TOKEN_COLORS.computeIfPresent(tokenType,
                    (k, currentScheme) -> new ColorScheme(currentScheme.foreground(), currentScheme.background(),
                            currentScheme.bold(), italicCheckbox.isSelected(), currentScheme.underline(),
                            currentScheme.subscript(), currentScheme.superscript())));
            ...
            underlineCheckbox.addActionListener(e -> TOKEN_COLORS.computeIfPresent(tokenType,
                    (k, currentScheme) -> new ColorScheme(currentScheme.foreground(), currentScheme.background(),
                            currentScheme.bold(), currentScheme.italic(), underlineCheckbox.isSelected(),
                            currentScheme.subscript(), currentScheme.superscript())));
            ...
            subscriptCheckbox.addActionListener(e -> TOKEN_COLORS.computeIfPresent(tokenType,
                    (k, currentScheme) -> new ColorScheme(currentScheme.foreground(), currentScheme.background(),
                            currentScheme.bold(), currentScheme.italic(), currentScheme.underline(),
                            subscriptCheckbox.isSelected(), currentScheme.superscript())));
            ...
            superscriptCheckbox.addActionListener(e -> TOKEN_COLORS.computeIfPresent(tokenType,
                    (k, currentScheme) -> new ColorScheme(currentScheme.foreground(), currentScheme.background(),
                            currentScheme.bold(), currentScheme.italic(), currentScheme.underline(),
                            currentScheme.subscript(), superscriptCheckbox.isSelected())));
        ...
        copySourceButton.addActionListener(e -> {
            ...
            JOptionPane.showMessageDialog(dialog, "Color scheme copied to clipboard as Java source code.", "Copied",
    ...
    private String generateColorSchemeSource() {
        ...
        var entries = TOKEN_COLORS.entrySet().stream().sorted(Comparator.comparingInt(e -> e.getKey().ordinal()))
                .toList();
        for (int i = 0; i < entries.size(); i++) {
            var entry = entries.get(i);
            TokenType tokenType = entry.getKey();
            ColorScheme scheme = entry.getValue();
            String fg = scheme.foreground() != null ? String.format("0x%06x", scheme.foreground().getRGB() & 0xFFFFFF)
                    : "null";
            String bg = scheme.background() != null ? String.format("0x%06x", scheme.background().getRGB() & 0xFFFFFF)
                    : "null";
            String suffix = i < entries.size() - 1 ? ")," : ")";
            sb.append(String.format(
                    "%n            Map.entry(TokenType.%-16s new ColorScheme(%s, %s, %s, %s, %s, %s, %s)%s",
                    tokenType.name() + ",", fg, bg, scheme.bold(), scheme.italic(), scheme.underline(),
                    scheme.subscript(), scheme.superscript(), suffix));
        }
        sb.append("\n    );");
        return sb.toString();
    }
    ...
    /**
     * Thrown when an EDT call via {@link #callOnEDT} or {@link #runOnEDT} fails due
     * to thread interruption or an exception in the invoked code.
     */
    ...
    /**
     * Runs the given supplier on the EDT and returns its result. Wraps checked
     * exceptions into RuntimeException.
     */
    ...
    // Uses SafeLabelView to handle JDK bug where GlyphView children become stale
    // (zero-length element references) during rapid setCharacterAttributes calls.
    ...
    // LabelView that guards against stale zero-length elements (JDK bug in
    // GlyphView.getText).
    // Overrides getText() — the single source of all StateInvariantError throws —
    // so all
    // code paths (getPreferredSpan, getBreakWeight, paint, etc.) are protected.
    private static class SafeLabelView extends javax.swing.text.LabelView {
        private static final javax.swing.text.Segment EMPTY_SEGMENT = new javax.swing.text.Segment(new char[0], 0, 0);

        SafeLabelView(javax.swing.text.Element elem) {
            super(elem);
        }

        @Override
        public javax.swing.text.Segment getText(int p0, int p1) {
            return p1 <= p0 ? EMPTY_SEGMENT : super.getText(p0, p1);
        }
    }
