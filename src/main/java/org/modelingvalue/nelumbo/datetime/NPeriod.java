...
public final class NPeriod extends Node {
    ...
    // Left (date) branch: get(0)'s elements are themselves [num,unit] pairs. Right
    // (time-only) branch:
    ...
    // Each unit must advance strictly along its canonical order, so repeats (==
    // last) and out-of-order
