        ...
        do {
            ...
            for (AndOrImpl andOr : now) {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    bound = InferResult.bind(predResult[i].facts(), predDecl[i], andOr, declaration).addAll(//
                            InferResult.bind(predResult[i].falsehoods(), predDecl[i], andOr, declaration)).remove(andOr);
                    if (bound.isEmpty()) {
