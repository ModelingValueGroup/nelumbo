...
public class ParseException extends Exception {
	...
	public ParseException(String s, List<AstElement> elements) {
		this(null, s, elements);
	}

	public ParseException(Throwable cause, String s, List<AstElement> elements) {
		this(cause, s, firstToken(elements), lastToken(elements));
	}
	...
	private static Token lastToken(AstElement... elements) {
		Token lastToken = elements.length > 0 ? elements[elements.length - 1].lastToken() : null;
		if (lastToken != null) {
	...
	private static Token firstToken(AstElement... elements) {
		Token firstToken = elements.length > 0 ? elements[0].firstToken() : null;
		if (firstToken != null) {
	...
	private ParseException(Throwable cause, String message, int line, int position, int index, int length,
			String fileName) {
		super(message, cause);
		this.line = line;
