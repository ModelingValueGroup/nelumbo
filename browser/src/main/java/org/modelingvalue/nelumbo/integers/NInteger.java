...
// OVERLAY of the core NInteger for the TeaVM (browser) build - delta: of(BigInteger) canonicalizes the value.
...
public final class NInteger extends Node {
    ...
    public static NInteger of(BigInteger val) {
        // canonicalize: the TeaVM classlib can produce equal BigIntegers (e.g. from add()) with
        // un-normalized digit arrays and therefore different hashCode()s
        return new NInteger(NodeInfo.of(FUNCTOR), new BigInteger(val.toByteArray()));
    }
